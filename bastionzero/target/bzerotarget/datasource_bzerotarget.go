			...
			&bzdatasource.SingleDataSourceConfigWithTimeout[bzeroTargetModel, targets.BzeroTarget]{
				BaseSingleDataSourceConfig: &bzdatasource.BaseSingleDataSourceConfig[bzeroTargetModel, targets.BzeroTarget]{
					...
					MarkdownDescription: baseDesc +
						"\n\nSpecify exactly one of `id` or `name`. When specifying a `name`, an error is triggered if more than one Bzero target is found. " +
						"This data source retries with exponential backoff (provide optional `timeouts.read` [duration](https://pkg.go.dev/time#ParseDuration) to control how long to retry. Defaults to 15 mintues.) until the Bzero target is found. " +
						"This is useful if there is a chance the target does not exist yet (e.g. the target is in the process of registering to BastionZero).",
				},
