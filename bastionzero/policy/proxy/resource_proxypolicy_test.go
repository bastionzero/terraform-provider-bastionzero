		...
		Steps: []resource.TestStep{
			{
				...
				Config:      testAccProxyPolicyConfigTargets("test", policy.FlattenPolicyTargets(context.Background(), []policies.Target{{ID: "foo", Type: "foo"}})),
		...
		Steps: []resource.TestStep{
			{
				...
				Config:      testAccProxyPolicyConfigSubjects("test", policy.FlattenPolicySubjects(context.Background(), []policies.Subject{{ID: "foo", Type: "foo"}})),
				ExpectError: regexp.MustCompile(`Invalid Attribute Value Match`),
			},
		},
	})
