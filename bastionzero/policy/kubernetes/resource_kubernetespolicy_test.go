...
func TestAccKubernetesPolicy_Clusters(t *testing.T) {
	...
	acctest.SkipIfNotInAcceptanceTestMode(t)
	acctest.PreCheck(ctx, t)
	acctest.FindNClusterTargetsOrSkipAsPolicyCluster(t, target1, target2)
		...
		Steps: []resource.TestStep{
			{
				...
				Config:      testAccKubernetesPolicyConfigSubjects("test", policy.FlattenPolicySubjects(context.Background(), []policies.Subject{{ID: uuid.New().String(), Type: "foo"}})),
				ExpectError: regexp.MustCompile(`Invalid Attribute Value Match`),
			},
			{
				// Invalid ID not permitted
				Config:      testAccKubernetesPolicyConfigSubjects("test", policy.FlattenPolicySubjects(context.Background(), []policies.Subject{{ID: "foo", Type: subjecttype.User}})),
				ExpectError: regexp.MustCompile(`Invalid Attribute Value Match`),
			},
		},
	})
