...
func getValuesCheckMap(serviceAccount *serviceaccounts.ServiceAccount) map[string]string {
	...
	if serviceAccount.LastLogin != nil {
		valuesCheckMap["last_login"] = serviceAccount.LastLogin.UTC().Format(time.RFC3339)
	} else {
		...
		Steps: []resource.TestStep{
			{
				...
				// the list
				Check: resource.TestCheckTypeSetElemNestedAttrs(dataSourceName, "service_accounts.*", getValuesCheckMap(serviceAccount)),
			},
		},
	})
}
